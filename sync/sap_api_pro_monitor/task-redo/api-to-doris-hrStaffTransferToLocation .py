...
apiName = 'HR_Staff_Transfer_To_Location'
...
HRStaffTransferToLocationTableNameDwd = "dwd_hr_staff_transfer_to_location_day_ef"
HRStaffTransferToLocationTableNameOds = "ods_hr_staff_transfer_to_location_day_ei"
HRStaffTransferToLocationTableNameOdsDetail = "ods_hr_staff_transfer_to_location_detail_day_ei"
...
def syncApiData(user_name, password, query_url, page_index=1, start_time='', end_time=''):
    ...
    start = time.time()
    print("查询时间：", start_time, "-", end_time)
...
def deal_ods_detail(response, query_url, page_index=1):
    ...
    insert_count = tmp_dataframe.to_sql(HRStaffTransferToLocationTableNameOdsDetail, tarEngine, if_exists='append',
                                        index=False)
    print(f"{HRStaffTransferToLocationTableNameOdsDetail}数据插入成功，受影响行数：", insert_count)
...
def deal_ods(response, query_url, page_index):
    ...
    insert_count = tmp_dataframe.to_sql(HRStaffTransferToLocationTableNameOds, tarEngine, if_exists='append',
                                        index=False)
    print(f"{HRStaffTransferToLocationTableNameOds}数据插入成功，受影响行数：", insert_count)
...
def deal_dwd(response):
    ...
    try:
        ...
        insert_count = result_dataframe.to_sql(HRStaffTransferToLocationTableNameDwd, tarEngine, if_exists='append',
                                               index=False)
        ...
        if '5025' in str(e.__dict__['orig']).split(',')[0]:
            ...
            if len(over_length_dataframe) != 0:
                ...
                insert_count = result_dataframe.to_sql(HRStaffTransferToLocationTableNameDwd, tarEngine,
                                                       if_exists='append',
                                                       index=False)
            else:
                raise e
            ...
            raise e
    print(
        f"{HRStaffTransferToLocationTableNameDwd}数据插入成功，总数据条数: {len(result_dataframe)}，插入行数：{insert_count}")


def check_repeat_data(dataframe_org):
    worker_id_list = "','".join(dataframe_org['worker_id'].to_list())
    sql = text(
        f"SELECT {','.join(list(fieldMapping.values()))} FROM {HRStaffTransferToLocationTableNameDwd} WHERE worker_id IN ('{worker_id_list}')")
...
if __name__ == '__main__':
    ...
    # transfer_to_location_url = "https://pwc-dev.it-cpi010-rt.cpi.cn40.apps.platform.sapcloud.cn/http/vprofile/transfer_to"
    transfer_to_location_url = "https://pwc.it-cpi010-rt.cpi.cn40.apps.platform.sapcloud.cn/http/vprofile/transfer_to"
    if startTimeStr == '':
        syncApiData(user, pwd, transfer_to_location_url)
    else:
        ...
        while startTime < endTime:
            ...
            syncApiData(user, pwd, transfer_to_location_url, start_time=tmpStartTime.strftime("%Y-%m-%dT%H:%M:%S"),

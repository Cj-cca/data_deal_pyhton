...
def syncDataWithIndex(src_engine, src_table, tar_engine, tar_table, unique_index, schema):
    srcConn = src_engine.connect()
    ...
    with MyThreadPoolExecutor(max_workers=4) as t:
        for i in range(0, data_count, gap):
            sql = text(f"""select * from {src_table} where {unique_index}>(
    select security_id from {src_table} order by {unique_index} limit {i},1
    ) limit {gap}""")
            t.submit(writeDate, (src_engine, tar_engine, sql, tar_table, schema))
    print("data insert is complete")
...
def runSyncDataWithIndex():
    # sqlserver
    srcTable = "ces_new.dwd_security_details_day_ei"
    tarTable = "SecurityDetails"
    schema = "dbo"
    srcEngine = getEngine("10.158.35.241", "admin_user", "6a!F@^ac*jBHtc7uUdxC", 9030, EngineType.mysql,
                          "ces_new")
    tarEngine = getEngine("CNSHASQLPWV765", "CESSecurity_PowerBI_RW", "U0OVt9lVSYxaXx7Q", 1800, EngineType.sqlserver,
                          "CESSecurity_PowerBI")
    # tarEngine = getEngine("10.158.34.175", "root", "", 9030, EngineType.mysql, "PwCMDM")
    uniqueIndex = "security_id"
    syncDataWithIndex(srcEngine, srcTable, tarEngine, tarTable, uniqueIndex, schema)

...
tarEngine = create_engine('mysql+pymysql://root@10.158.16.244:9030/work_day_stage')
batchID = 0
avgCostTime = 0
maxCostTime = 0
...
def deal_ods(response, query_url, page_index):
    ...
    insert_count = tmp_dataframe.to_sql(HRStaffTransferToLocationTableNameOds, tarEngine, if_exists='append', index=False)
...
def deal_dwd(response):
    ...
    insert_count = result_dataframe.to_sql(HRStaffTransferToLocationTableNameDwd, tarEngine, if_exists='append', index=False)
    print(f"{HRStaffTransferToLocationTableNameDwd}数据插入成功，总数据条数: {len(result_dataframe)}，插入行数：{insert_count}")
